//Importa o Estilo de CSS e a Função Jquery JS...
...
/**************************************************************************************/
//Essas funções tem a idéia de facilitar na hora de Clique Arraste do "DragDrop" ...
function ocultar_div() {
    //Essa DIV "html5lightbox-web" equivale ao Iframe que é carregado dentro da DIV "html5-lightbox-box" Principal ...
    document.getElementById('html5lightbox-web').style.visibility   = 'hidden';
    document.getElementById('html5-elem-box').style.height          = 20+'px';//Essa DIV "html5-elem-box" está dentro da DIV "html5lightbox-web" Principal ...
}
...
function loadHtml5LightBox(jsFolder) {
    (function($) {
        $.fn.html5lightbox = function(options) {
            ...
            inst.options = $.extend({freelink: "http://html5box.com/", defaultvideovolume: 1, autoclose: false, insideiframe: false, autoplay: true, loopvideo: false, html5player: true, responsive: true, nativehtml5controls: false, videohidecontrols: false, nativecontrolsonfirefox: true, nativecontrolsonie: true, imagekeepratio: true, useflashonie9: true, useflashonie10: true, useflashonie11: false, googleanalyticsaccount: "", arrowloop: true, shownavigation: true,
                ...
                version: "3.3", stamp: true, freemark: "72,84,77,76,53,32,76,105,103,104,116,98,111,120,32,70,114,101,101,32,86,101,114,115,105,111,110", watermark: "", watermarklink: ""}, options);
            ...
            if ($("div#html5lightbox_general_options").length)
                $.each($("div#html5lightbox_general_options").data(),
                        function(key, value) {
                            inst.options[key.toLowerCase()] = value
                        });
            ...
            if ((inst.options.isIE6789 ||
                    inst.options.isIE10 || inst.options.isIE11) && inst.options.nativecontrolsonie)
                inst.options.nativehtml5controls = true;
            ...
            inst.createMarkup = function() {
                ...
                if (inst.options.borderradius > 0) {
                    inst.$elem.css({"border-radius": inst.options.borderradius + "px", "-moz-border-radius": inst.options.borderradius + "px", "-webkit-border-radius": inst.options.borderradius +
                                "px"});
                    if (inst.options.titlestyle == "inside")
                        ...
                        inst.$elemWrap.css({"border-top-left-radius": inst.options.borderradius + "px", "-moz-top-left-border-radius": inst.options.borderradius + "px", "-webkit-top-left-border-radius": inst.options.borderradius + "px", "border-top-right-radius": inst.options.borderradius +
                                    "px", "-moz-top-right-border-radius": inst.options.borderradius + "px", "-webkit-top-right-border-radius": inst.options.borderradius + "px"});
                ...
                if (inst.options.showsocial) {
                    ...
                    if (inst.options.showfacebook)
                        socialCode += '<div class="html5-social-btn' +
                    ...
                    if (inst.options.showtwitter)
                        socialCode += '<div class="html5-social-btn' + (inst.options.socialrotateeffect ? " html5-rotate" : "") + ' html5-social-twitter" style="' + socialBtnCSS + '"><div class="icon-twitter" style="' + socialCSS + 'background-color:#03b3ee;"></div></div>';
                    ...
                    $(".html5-social-btn", inst.$lightbox).click(function() {
                        ...
                        if ($(this).hasClass("html5-social-facebook"))
                            ...
                            window.open("https://pinterest.com/pin/create/bookmarklet/?media=" + encodeURIComponent(shareMedia) + "&url=" + encodeURIComponent(shareUrl) + "&description=" + encodeURIComponent(shareTitle) + "&is_video=" + (isVideo ?
                                    "true" : "false"), "_blank");
                        return false
                    })
                }
                inst.$watermark = $("#html5-watermark", inst.$lightbox);
                //Comentei esse trecho abaixo para que não me mostre a propaganda de site p/ clicar no HTML5Light.com ...
            ...
            inst.showTitle = function(w, t, description) {
                ...
                if (inst.options.showdescription && description && description.length > 0)
                    text += '<p class="html5-description">' +
                            description + "</p>";
                inst.$text.html(text);
                ...
                var sizeObj = inst.calcElemSize({w: elemW,
                    h: elemH}, inst.options.imagekeepratio);
            ...
            inst.showYoutubeVimeo = function(elem) {
                ...
                inst.resizeLightbox(dataW, dataH, true, function() {
                    ...
                    if (elem[ELEM_TYPE] == 3 && typeof YT === "object" && typeof YT.Player === "function")
                        new YT.Player("html5boxiframevideo", {events: {"onStateChange": function(event) {
            ...
            inst.resizeWindow = function() {
                ...
                if (inst.options.responsivebarheight) {
                    if (winH <=
                            inst.options.smallscreenheight)
                        inst.options.barheight = inst.options.barheightonsmallheight;
                    else
            ...
            inst.resizeLightbox = function(elemW, elemH, bAnimate, onFinish) {
                ...
                if (inst.options.nextElem <= inst.options.curElem)
                    if (inst.options.onlastitem &&
                            window[inst.options.onlastitem] && typeof window[inst.options.onlastitem] == "function")
            ...
            inst.reset = function() {
                ...
                if (inst.options.titlestyle ==
                        "bottom" || inst.options.titlestyle == "inside")
                    inst.$elemData.hide();
                if (!inst.options.fullscreenmode)
                    inst.$close.hide();
            ...
            inst.enableSwipe = function() {
                inst.$elem.html5lightboxTouchSwipe({preventWebBrowser: false, swipeLeft: function() {
                        inst.gotoSlide(-1)
                    }, swipeRight: function() {
                        inst.gotoSlide(-2)
                    }})
    ...
    (function($) {
        $.fn.lightboxHTML5VideoControls = function(skinFolder, parentInst, hidecontrols, hideplaybutton, defaultvolume) {
            ...
            $videoBtnPause.css({display: "none", position: "absolute", top: 0, left: 0, width: BUTTON_SIZE +
                        "px", height: BUTTON_SIZE + "px", cursor: "pointer", "background-image": "url('" + skinFolder + "html5boxplayer_playpause.png" + "')", "background-position": "top right"}).hover(function() {
            ...
            $videoTimeTotal.css({display: "block", position: "relative", "float": "right", "line-height": BAR_HEIGHT +
                        "px", "font-weight": "normal", "font-size": "12px", margin: "0 8px", "font-family": "Arial, Helvetica, sans-serif", color: "#fff"});
            ...
            $videoSeekerPlay.css({display: "block", position: "absolute", left: 0, top: 0,
                height: "100%", "background-color": "#fcc500"});
            if (!isIPhone && ($videoObj.get(0).requestFullscreen || $videoObj.get(0).webkitRequestFullScreen || $videoObj.get(0).mozRequestFullScreen || $videoObj.get(0).webkitEnterFullScreen || $videoObj.get(0).msRequestFullscreen)) {
                ...
                $videoFullscreen.css({display: "block", position: "relative", "float": "right", width: BUTTON_SIZE + "px", height: BUTTON_SIZE + "px", margin: Math.floor((BAR_HEIGHT - BUTTON_SIZE) / 2), cursor: "pointer", "background-image": "url('" +
                            skinFolder + "html5boxplayer_fullscreen.png" + "')", "background-position": "left top"}).hover(function() {
            ...
            if (hd) {
                ...
                $videoHD.css({display: "block", position: "relative", "float": "right", width: BUTTON_SIZE + "px", height: BUTTON_SIZE + "px", margin: Math.floor((BAR_HEIGHT - BUTTON_SIZE) / 2), cursor: "pointer", "background-image": "url('" + skinFolder + "html5boxplayer_hd.png" + "')", "background-position": (isHd ? "right" : "left") + " center"}).bind(eClick, function() {
                    isHd = !isHd;
                    $(this).css({"background-position": (isHd ?
                                "right" : "left") + " center"});
            ...
            if ($videoObj.get(0).volume === volume / 2 + 0.1) {
                ...
                $videoVolumeBar.css({display: "none", position: "absolute", left: 4, bottom: "100%", width: 24, height: 80, "margin-bottom": Math.floor((BAR_HEIGHT - BUTTON_SIZE) / 2), "background-color": "#000000",
                ...
                $videoVolumeBarBg.bind(eStart, function(e) {
                    ...
                    $videoVolumeBarActive.css({height: Math.round(vol *
                                100) + "%"});
    ...
    jQuery(document).ready(function() {
        if (typeof html5Lightbox === "undefined")
            ...
            alert(document.getElementById('html5-lightbox-box'))
        
            Drag.init(document.getElementById('html5-lightbox-box'))//Representa a parte Branca da Div que carrega o conteúdo que está no Iframe ...
    })
};