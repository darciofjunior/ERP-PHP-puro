...
for($i = $inicio; $i < $fim; $i++) {
    ...
    if($i != 0 && ($i % 5 == 0)) {
        ...
        if($linha_atual <= 2) {
            ...
            $enter+= 17.6;
        }else if($linha_atual == 10) {
            $enter+= 17.7;
        }else if($linha_atual <= 12) {
            $enter+= 18;
        }else if($linha_atual <= 13) {
            $enter+= 16;
        }
        $linha_atual++;
    }
    ...
    if($_POST['txt_pular'] > $i) {
        ...
        if(!empty($campos[0]['desenho_para_etiqueta'])) {                    
            $pdf->Image('../../../../imagem/desenhos_grupos_pas/'.$campos[0]['desenho_para_etiqueta'], $tab - 2, $enter - 7.8, 26.8, 6, 'JPG');
        }                
        ...
        if(empty($campos[0]['codigo_barra']) || $campos[0]['codigo_barra'] == '0000000000000') {
            ...
            $pdf->EAN13($tab, $enter + 4.8, $campos[0]['codigo_barra'], '3', '.21');
        }
        /***************************************************************/
    }
    $tab+= 34.2;
