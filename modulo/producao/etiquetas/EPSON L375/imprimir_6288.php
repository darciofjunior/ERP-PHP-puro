...
for($i = $inicio; $i < $fim; $i++) {
    ...
    if($_POST['txt_pular'] > $i) {
        ...
        if(!empty($campos[0]['desenho_para_etiqueta'])) {
            $pdf->Image('../../../../imagem/desenhos_grupos_pas/'.$campos[0]['desenho_para_etiqueta'], $tab - 3, $enter - 7.8, 107, 47, 'JPG');
        }
        ...
        if(empty($campos[0]['codigo_barra']) || $campos[0]['codigo_barra'] == '0000000000000') {
            $pdf->Text($tab + 32, $enter + 62, '');
        }else {//Printa o Código de Barra normal ...
            $pdf->EAN13($tab + 32, $enter + 62, $campos[0]['codigo_barra'], '12', '.30');
        }
        /***************************************************************/
    }
    $tab+= 120;
