...
$formato_papel	= 'etiqueta_6288'; // A3, A4, A5, Letter, Legal
...
for($i = $inicio; $i < $fim; $i++) {
	...
	if($_POST['txt_pular'] > $i) {
		...
		for($j = 0; $j < strlen($campos[0]['discriminacao']); $j++) {
			if($j <= 30) {//Essa Discriminação será printada na 1ª Linha da Etiqueta ...
		...
		if(!empty($campos[0]['desenho_para_etiqueta'])) {
                    $pdf->Image('../../../../imagem/desenhos_grupos_pas/'.$campos[0]['desenho_para_etiqueta'], $tab + 5, $enter - 7.8, 107, 47, 'JPG');
		}
		$pdf->Text($tab, $enter + 46, $pcs_embalagem.ucfirst($discriminacao1));
		$pdf->Text($tab, $enter + 53, $discriminacao2);
