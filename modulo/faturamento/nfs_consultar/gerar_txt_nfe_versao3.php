...
if($id_nf > 0) {
    ...
    $forma_pagamento            = ($campos_nfs[0]['vencimento1'] == 0) ? 0 : 1;//À vista ou a Prazo ...
    ...
    $forma_pagamento 	= ($campos_nfso[0]['vencimento1'] == 0) ? 0 : 1;//À vista ou a Prazo ...
...
$texto.= 'B|35||'.strtr(strtoupper($cfop_descritivo), $caracteres_invalidos, $caracteres_validos).'|'.$forma_pagamento.'|55|1|'.$numero_nf.'|'.$data_emissao.'T'.date('H:i').':00-03:00|'.$data_saida.'|'.$tipo_nf.'|'.$destino_operacao.'|3550308|1|1||1|'.$finalidade_emissao.'|'.$consumidor_final.'|0|3|3.10.54|||'.$quebrar_linha;
...
Cliente pediu ...***/
        if(!empty($campos_itens[$i]['id_produto_acabado_discriminacao'])) {
            ...
            $codigo_barra_trib          = ($campos_itens[$i]['icms'] == 0) ? '' : $campos_pa_substitutivo[0]['codigo_barra'];
...
//Aqui foi feito a Compra de um PA Normal, não foi feito o Gato por Lebre ...
            ...
            $codigo_barra_trib          = ($campos_itens[$i]['icms'] == 0) ? '' : $campos_itens[$i]['codigo_barra'];
        }
/********************************************************************************************************/
        ...
        if($id_pais != 31 || $campos_nfs[0]['trading'] == 1) {//Clientes Estrangeiros "Exportação" ...
            if($campos_itens[$i]['id_unidade'] == $campos_itens[$i]['id_unidade_tributavel']) {//Unidades Iguais ...
                ...
                if($campos_itens[$i]['id_unidade_tributavel'] == 1) {//Se for Kilo ...
                    //Faço arredondamento p/ 4 casas porque o sistema da Sefaz trabalha dessa forma, com 4 casas p/ Qtde e 4 casas p/ Valor ...
                    $qtde_nota_un_tributaria    = round($campos_itens[$i]['peso_unitario'] * $campos_itens[$i]['qtde_nota'], 4);
                    $valor_item_un_tributaria   = round($campos_itens[$i]['valor_item'] / $campos_itens[$i]['peso_unitario'], 4);
...
/**************************************************************************************/
            ...
            /*******************************************************************************************/
            /*****************CFOP. 5405 - Discriminação de itens apenas para o Barzel******************/
            /*******************************************************************************************/
            if($cfop == '5.405' && $campos_nfs[0]['id_cliente'] == 118) {//Nessa CFOP, a situação é um pouquinho diferente ...
                ...
                $base_calculo_icms_st_item  = $vetor_dados_substituicao_tributaria['base_calculo_icms_st_item_current_rs'];
                $valor_icms_st_item         = $vetor_dados_substituicao_tributaria['valor_icms_st_item_current_rs'];
            }
            /****************************************************************************************************/
            ...
            if($situacao_tributaria == '00') {//Tributada Integralmente ...
                ...
                $texto.= 'N08|'.$campos_itens[$i]['origem_mercadoria'].'|'.$situacao_tributaria.'|'.number_format($base_calculo_icms_st_item, 2, '.', '').'|'.number_format($valor_icms_st_item, 2, '.', '').$quebrar_linha;
            }else if($situacao_tributaria == '90') {//Outras ...
                $texto.= 'N10|'.$campos_itens[$i]['origem_mercadoria'].'|'.$situacao_tributaria.'|3|'.number_format($campos_itens[$i]['reducao'], 2, '.', '').'|'.number_format($base_calculo_icms_item, 2, '.', '').'|'.number_format($campos_itens[$i]['icms'], 2, '.', '').'|'.number_format($valor_icms_item, 2, '.', '').'|4|'.$iva_layout.'||'.number_format($base_calculo_icms_st_item, 2, '.', '').'|'.number_format($campos_itens[$i]['icms_intraestadual'], 2, '.', '').'|'.number_format($valor_icms_st_item, 2, '.', '').$quebrar_linha;
            }
/**************************************************************************************/
...
/****************************************Difal*****************************************/
/**************************************************************************************/
        if($calculo_impostos_item['difal'] > 0) $texto.= 'NA|'.number_format($base_calculo_icms_item, 2, '.', '').'|0.00|'.$campos_itens[$i]['icms_intraestadual'].'|'.$campos_itens[$i]['icms'].'|80|0.00|'.number_format($calculo_impostos_item['valor_icms_destino'], 2, '.', '').'|'.number_format($calculo_impostos_item['valor_icms_remetente'], 2, '.', '').$quebrar_linha;
    }
    /**************************************************************************************/
    ...
    $texto.= 'W02|'.number_format($calculo_total_impostos['base_calculo_icms'], 2, '.', '').'|'.number_format($calculo_total_impostos['valor_icms'], 2, '.', '').'|0.00|'.number_format($calculo_total_impostos['base_calculo_icms_st'], 2, '.', '').'|'.number_format($calculo_total_impostos['valor_icms_st'], 2, '.', '').'|'.number_format($calculo_total_impostos['valor_total_produtos'], 2, '.', '').'|'.number_format($calculo_total_impostos['valor_frete'], 2, '.', '').'|0.00|'.$desconto.'|0.00|'.number_format(round($calculo_total_impostos['valor_ipi'], 2), 2, '.', '').'|'.number_format($calculo_pis, 2, '.', '').'|'.number_format($calculo_cofins, 2, '.', '').'|'.number_format($calculo_total_impostos['outras_despesas_acessorias'], 2, '.', '').'|'.number_format(round($calculo_total_impostos['valor_total_nota'], 2), 2, '.', '').$quebrar_linha;
    /**************************************************************************************/
    /*******************************Layout - Dados de ISSQN *******************************/
    /**************************************************************************************/
    ...
    for($i = 0; $i < $qtde_vias_duplic; $i++) {
        $texto.= 'Y07|'.$duplicata_array[$i].'|'.data::datatodate($data_vencimento_array[$i], '-').'|'.number_format($valor_duplicata[$i], 2, '.', '').$quebrar_linha;
...
/*****************************************************************************************************/
    ...
    for($i = 0; $i < count($vetor_cfops_da_nf); $i++) {
        ...
        if($final_cfop == '.401') {//PA's com IVA - OF (Industrial)
...
pelo caminho desse Script se esta CFOP for pertinente a uma Nota Fiscal Complementar ...*/
    ...
    /**************************************************************************************/	
            ...
            $texto.= 'I18|'.$numero_di.'|'.$data_registro.'|SANTOS|SP|'.date('Y-m-d').'|1|'.$despesas_acessorias.'|1|||'.$campos_cliente[0]['cod_cliente'].'|'.$quebrar_linha;
    ...
    /**************************************************************************************/
            ...
            //40 - Isenta ou 41 - Não tributada ou 50 - Suspensão ...
                ...
                $texto.= 'N08|'.$campos_itens[$i]['origem_mercadoria'].'|'.$situacao_tributaria.'|0.00|0.00'.$quebrar_linha;
            }else if($situacao_tributaria == '90') {//Outras ...
...
/*****************************************************************************************************/
    ...
    /**************************************************************************************/	
            ...
            $texto.= 'I|'.$referencia.'||'.strtr(strtoupper($discriminacao), $caracteres_invalidos, $caracteres_validos).'|'.str_replace('.', '', $campos_itens[$i]['classific_fiscal']).'||'.str_replace('.', '', $dados_produto['cfop']).'|'.$unidade.'|'.number_format($campos_itens[$i]['qtde_nota'], 4, '.', '').'|'.number_format($campos_itens[$i]['valor_item'], 8, '.', '').'|'.number_format($campos_itens[$i]['qtde_nota'] * $campos_itens[$i]['valor_item'], 2, '.', '').'||'.$unidade.'|'.number_format($campos_itens[$i]['qtde_nota'], 4, '.', '').'|'.number_format($campos_itens[$i]['valor_item'], 8, '.', '').'|'.$frete_despesas_acessorias_item_layout.'|||'.$despesas_acessorias.'|'.$indTot.'|||'.$quebrar_linha;
            //CEST ...
            if(!empty($campos_itens[$i]['cest'])) $texto.= 'I05c|'.str_replace('.', '', $campos_itens[$i]['cest']).'|'.$quebrar_linha;
    ...
    $texto.= 'W|'.$quebrar_linha;
    $texto.= 'W02|'.number_format($base_calculo_icms, 2, '.', '').'|'.number_format($valor_icms, 2, '.', '').'|0.00|'.number_format($calculo_total_impostos['base_calculo_icms_st'], 2, '.', '').'|'.number_format($calculo_total_impostos['valor_icms_st'], 2, '.', '').'|'.number_format($calculo_total_impostos['valor_total_produtos'], 2, '.', '').'|'.number_format($calculo_total_impostos['valor_frete'], 2, '.', '').'|0.00|'.$desconto.'|'.number_format($total_ii_itens_rs, 2, '.', '').'|'.number_format(round($calculo_total_impostos['valor_ipi'], 2), 2, '.', '').'|'.number_format($total_pis_itens_rs, 2, '.', '').'|'.number_format($total_cofins_itens_rs, 2, '.', '').'|'.number_format($calculo_total_impostos['outras_despesas_acessorias'], 2, '.', '').'|'.number_format(round($calculo_total_impostos['valor_total_nota'], 2), 2, '.', '').$quebrar_linha;
