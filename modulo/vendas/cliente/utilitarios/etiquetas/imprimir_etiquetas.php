...
}else {//Busca do Nome do Departamento da Própria Combo de Departamento selecionada ...
...
//3ª Linha da Etiquetagem	
    ...
    }else {//Não tem como fazer o SQL de quem é o Contato do Cliente, se o Usuário ao menos não selecionou o Depto. ...
        if($_POST['chkt_preencher_ignorar_depto'] == 1) {
            //Aqui é a Primeira Coluna de Etiquetas, Coluna Esquerda ...
            $pdf->Cell(100, 5, 'A/C DEPTO. '.$departamento.' - '.strtoupper($_POST['txt_contato']), 0, 0, 'L');
            //Aqui é a Segunda Coluna de Etiquetas, Coluna Direita ...
            $pdf->Cell(100, 5, 'A/C DEPTO. '.$departamento.' - '.strtoupper($_POST['txt_contato']), 0, 1, 'L');
        }else {//Aqui é o único caso em que temos 4 linhas p/ a etiqueta ...
            //Aqui é a Primeira Coluna de Etiquetas, Coluna Esquerda ...
...
//4ª Linha da Etiquetagem			
            $sql = "SELECT SUBSTRING_INDEX(nome, ' ', 2) AS nome 
                    ...
                    AND `ativo` = '1' LIMIT 1 ";
            $campos_departamento = bancos::sql($sql);
            if(count($campos_departamento) == 1) {//Busca o Contato do Cliente que está no Loop de acordo com Depto. que foi selecionado na combo ...
