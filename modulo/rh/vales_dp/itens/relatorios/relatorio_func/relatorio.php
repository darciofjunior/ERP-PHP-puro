...
/*Através da Data de Holerith eu busco qual é o id Data desta data de Holerith, vou utilizar esse id na 
parte de Holeriths ...*/
...
/*Busca de Funcionários na Data de Holerith e Desconto de Vale passado por parâmetro, "todas as Empresas" 

Obs: Só listo os usuários que possuem a Data de Admissão menor do que a Data de Holerith - porque senão dará erro
no Sistema, um funcionário admitindo recentemente jamais terá um holerith de 2, 3 anos atrás por exemplo ... */
if($opt_descontar == 'PF') {
...
//Busca de Funcionários no Desconto de Vale passado por parâmetro, "exceto a Empresa Grupo" ...
}else {
/*Listagem de Funcionários que ainda estão trabalhando ...
* Só não exibo os funcionários Default (1,2), ADAMO 91 e DIRETO BR 114 e os 
diretores Roberto 62, Dona Sandra 66 e Wilson 68 porque estes não são funcionários, simplesmente 
só possuem cadastrado no Sistema p/ poder acessar algumas telas ... 
//and f.id_funcionario not in (1, 2, 62, 66, 68, 91, 114) */
...
$campos = bancos::sql($sql);
$linhas = count($campos);
//Se não existir nenhum Vale então retorno essa mensagem informando ao usuário ...
if($linhas == 0) {
    $pdf->SetFont('Arial', 'B', 9);
    $pdf->Cell($GLOBALS['ph'] * 90, 4.5, 'NÃO EXISTE(M) VALE(S) NESSA DATA DE HOLERITH ESPECIFICADA.', 0, 1, 'C');
//Se existir pelo menos 1 Vale ...
}else {
    $id_empresa_antigo              = '';
    $holeriths_impressos_por_pagina = 0;
    
    /*Aqui nesse array eu guardo quais os 2 maiores funcionários "Representantes" 
    ...
    //Disparo do Loop ...
    for($i = 0; $i < $linhas; $i++) {
        //Busca de Todos os Vale(s) do Funcionário do Loop na Data de Holerith e Desconto de Vale especificado ...
...
//Aqui eu trago a Comissão de acordo com a Empresa
...
//Só na 1ª vez que eu igualo a variável de Empresa p/ não dar problema ...
        if($i == 0) $id_empresa_antigo = $campos[$i]['id_empresa'];
//Comparo se já foi trocada 
        if($id_empresa_antigo != $campos[$i]['id_empresa']) {//Empresa diferente ...
//Aki significa que mudou para outra Empresa, e sendo assim solicito uma nova página ...
            $id_empresa_antigo = $campos[$i]['id_empresa'];

            //Significa que já foram impressos 2 registros daquela página, e sendo assim solicito uma nova página ...
            if($holeriths_impressos_por_pagina == 2) {
                $holeriths_impressos_por_pagina = 0;
                $pdf->AddPage();
            }else if($holeriths_impressos_por_pagina == 1) {
                //Aqui com esse comando eu mando fazer a impressão do Segundo Vale no meio da Folha
                $pdf->SetY(167.5);
                //Espaço entre um funcionário e outro ...
                $pdf->Cell($GLOBALS['ph']*90, 4.5, '-------------------------------------------------------------------------------------------------------------------------------------------------------------', 0, 1, 'L');
            }
        }else {//Ainda é a mesma Empresa
        ...
        if($opt_descontar == 'PD' || ($opt_descontar == 'PF' && ($campos[$i]['salario_pf'] + $campos[$i]['salario_premio'] + $outros_rend_prop > 0) || ($campos[$i]['salario_pd'] + $campos[$i]['salario_pf'] + $campos[$i]['salario_premio'] + $outros_rend_prop > 0 && $campos[$i]['id_empresa'] == 4) || ($comissao_pd + $comissao_pf > 0))) {
            ...
            //$pdf->SetFont('Arial', 'B', 9);
//N.º
            //$pdf->Cell($GLOBALS['ph']*3, 4.5, 'N.º ', 0, 0, 'L');
            //$pdf->SetFont('Arial', '', 9);
            $pdf->Cell($GLOBALS['ph']*20, 4.5, 'N.º '.$campos[$i]['codigo_barra'], 1, 0, 'L');
            //$pdf->SetFont('Arial', 'B', 9);
//Empresa
...
/***********************************************************************************************/
            ...
            $pdf->Cell($GLOBALS['ph'] * 30, 4.5, 'Discriminação', 1, 0, 'C', 1);
            $pdf->Cell($GLOBALS['ph'] * 30, 4.5, 'Data Emissão / Outros', 1, 0, 'C', 1);
            $pdf->Cell($GLOBALS['ph'] * 15, 4.5, 'Crédito R$', 1, 0, 'C', 1);
            $pdf->Cell($GLOBALS['ph'] * 15, 4.5, 'Débito R$', 1, 1, 'C', 1);
            $pdf->SetFont('Arial', '', 9);
            ...
            if(count($campos_funcionario_holerith) == 1) {
                if($opt_descontar == 'PF') {//Por Fora ...
                    ...
                    if($campos[$i]['tipo_salario'] == 1) {//Horista
//Quando o Tipo do Salário é Horista, então existe essa separação dos dias com as Horas ...
...
/*****************************************************************************************/
                        ...
                        //Quando a Empresa for Grupo, então eu tenho que calcular o Salário baseado no PD + PF
                        if($campos[$i]['id_empresa'] == 4) {//Grupo
                            $salario_bruto  = ($campos[$i]['salario_pd'] + $campos[$i]['salario_pf']);
                        }else {//Nas outras empresas, eu só me baseio PF ...
                        ...
                        //No cálculo PF, eu pego o "Salário do func" divido por 30 e multiplico pela qtde de Dias digitados ...
                    ...
                    //Quando o Valor for = 0, então eu não printo essa linha ...
                    if($calc_qtde_dias_hrs_trab_sal == 0) {
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                    }else if($calc_qtde_dias_hrs_trab_sal > 0) {//para Saber qual coluna Crédito ou Débito
...
/*****************************************************************************************/
                        ...
                        /*Igualei essa variável igual a de cima, p/ que não dê erro no relatório, essa variável não existia 
                        no PD, Dárcio*/
                        $calc_qtde_dias_hrs_trab_sal    = $calc_qtde_dias_hrs_trab;
                    }else {//Mensalista
                        $texto = 'Qtde Dia(s) Trabalhado(s)';
                        ...
                        $calc_qtde_dias_hrs_trab        = $salario_bruto;
                        //No cálculo PD, eu pego o "Salário do func" divido por 30 e multiplico pela qtde de Dias digitados ...
                        $calc_qtde_dias_hrs_trab_sal    = ($calc_qtde_dias_hrs_trab / 30) * $dias_horas_trabalhadas;
                    }
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, $texto, 1, 0, 'L');
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, $dias_horas_trabalhadas, 1, 0, 'C');
...
/***********************************************************************************************/
                //Se o Vale foi feito por Fora então busco a comissão do que foi feito por Fora ...
                if($opt_descontar == 'PF') {
                    if($campos[$i]['id_cargo'] == 47) {//Se o Cargo = Representante Interno, então ...
                ...
                }else {//Se o Vale foi feito por Dentro então busco a comissão do que foi feito por Dentro ...
                    if($campos[$i]['id_cargo'] == 47) {//Se o Cargo = Representante Interno, então ...
                        $comissao_rs = 0;//A comissão passa a ser Zerada ...
...
//Quando o Valor for = 0, então eu não printo essa linha ...
                    if($calc_qtde_faltas == 0) {
                        $pdf->Cell($GLOBALS['ph'] * 15, 4.5, '', 1, 0, 'R');
                        $pdf->Cell($GLOBALS['ph'] * 15, 4.5, '', 1, 1, 'R');
                    }else if($calc_qtde_faltas > 0) {//para Saber qual coluna Crédito ou Débito
...
//3) Qtde Atrasos
                $hora_qtde_atrasos = strtok($atrasos_hr_min, '.');//Pega até a 1ª ocorrência encontrada
                $minuto_qtde_atrasos = substr(strchr($atrasos_hr_min, '.'), 1);//Pega a partir 1ª ocorrência encontrada
                if($campos[$i]['tipo_salario'] == 1) {//Horista
                    $texto = 'Qtde Atrasos';
                ...
                if($opt_descontar == 'PF') {//Por Fora ...
                    ...
                    if($calc_qtde_atrasos == 0) {
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                    }else if($calc_qtde_atrasos > 0) {//para Saber qual coluna Crédito ou Débito
...
//4) DSR (sobre Faltas e Atrasos)
                if($campos[$i]['tipo_salario'] == 1) {//Horista
                    $hora_qtde_dsr = strtok($dsr_hr_min, '.');//Pega até a 1ª ocorrência encontrada
                    $minuto_qtde_dsr = substr(strchr($dsr_hr_min, '.'), 1);//Pega a partir 1ª ocorrência encontrada
                ...
                if($opt_descontar == 'PF') {//Por Fora ...
                    ...
                    if($calc_qtde_dsr == 0) {
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                    }else if($calc_qtde_dsr > 0) {//para Saber qual coluna Crédito ou Débito
...
//5) Hora Extra (Feriado ao Sábado) - A % da Hora Extra é de 30% em cima da hora normal ...
                if($opt_descontar == 'PF') {//Somente no Por Fora que aparece essa linha ...
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Hora Extra (Feriado ao Sábado)', 1, 0, 'L');
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, number_format($hora_extra_fer_sab, 2, ':', ''), 1, 0, 'C');

                    $so_hora_extra_fer_sab = strtok($hora_extra_fer_sab, '.');//Pega até a 1ª ocorrência encontrada
                    $so_min_extra_fer_sab = substr(strchr($hora_extra_fer_sab, '.'), 1);//Pega a partir 1ª ocorrência encontrada
                    $nova_hora_extra_fer_sab = ($so_hora_extra_fer_sab + $so_min_extra_fer_sab / 60);

                    //Se o funcionário trabalhou num feriado de sábado, então ele ganha 50% do valor de sua hora Extra ...
                    $aliquota_hora_extra = ($hora_extra_fer_sab > 0) ? 1.50 : 1.30;//Normal é 30% ...
                    ...
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                }else {//No PD, eu somente exibo as horas lançadas ...
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Hora Extra (Feriado ao Sábado)', 1, 0, 'L');
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, number_format($hora_extra_fer_sab, 2, ':', ''), 1, 0, 'C');
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
...
//Aqui mostra independente de ser PD ou PF ...
//6) Comissão
                $comissao_rs-=$dsr_rs;//Aqui nessa parte eu só preciso descontar o DSR ...
                $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Comissão', 1, 0, 'L');
//Quando o Valor for = 0, então eu não printo essa linha ...
...
//Quando o Valor for = 0, então eu não printo essa linha ...
                if($comissao_rs == 0) {
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                }else if($comissao_rs > 0) {//para Saber qual coluna Crédito ou Débito
...
//7) DSR Comissão
                $pdf->Cell($GLOBALS['ph']*30, 4.5, 'DSR Comissão', 1, 0, 'L');
                $pdf->Cell($GLOBALS['ph']*30, 4.5,'', 1, 0, 'C');
//Quando o Valor for = 0, então eu não printo essa linha ...
                if($dsr_rs == 0) {
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                }else if($dsr_rs > 0) {//para Saber qual coluna Crédito ou Débito
                ...
                $saldo_garantial_salarial = 0;//Zero essa variável p/ não herdar valores do Loop Anterior ...
//8) Garantia Salarial
                if($opt_descontar == 'PF') {
...
/******************************************Desconto PD******************************************/
//Somente se for Desconto PD que eu exibo essa linha, ou se o Funcionário for da Empresa Grupo, pois se é Grupo não tem Registro ...
/***********************************************************************************************/
...
//9) Pensão Alimentícia
                    ...
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Pensão Alimentícia', 1, 0, 'L');
...
/*10) Mensalidade MetalCred - o sistema posterior a essa Data exibe essa linha fixo, porque não existiam 
...
dia 23/05/2013 ...*/
                    if($_GET['cmb_data_holerith'] < '2013-05-23') {
                        ...
                        if($valor_metalcred < 0) {//Só exibe essa linha se existir valor de Credito Consignado ...
...
/***********************************************************************************************/
                if($opt_descontar == 'PF') {
//11) Proporc 13º + FER
                    //Move mais para cima essa linha no dia 30/06/2015 devido os cálculos de Garantia Salarial ...
                    //$sub_total_sal_rec = $calc_qtde_dias_hrs_trab_sal + $calc_qtde_faltas + $calc_qtde_atrasos + $calc_qtde_dsr;
/*Só p/ os funcs Wilson Baldez, Wellington Baldez e Lucas Faria Marques q não existe esse cálculo ]= 0, 
p/ os outros funcionários normal ...*/
                    if($id_funcionario_loop == 100 || $id_funcionario_loop == 145 || $id_funcionario_loop == 167) {
                        $proporc_13_fer = 0;
                    }else {//Outros Funcionários ...
                        $proporc_13_fer = ($comissao_rs + $dsr_rs + $sub_total_sal_rec + $saldo_garantial_salarial) * 0.194;//Aqui é uma proporção do 13º
                    }
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Proporc 13º + FER', 1, 0, 'L');
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, '', 1, 0, 'C');
//Quando o Valor for = 0, então eu não printo essa linha ...
                    if($proporc_13_fer == 0) {
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                    }else if($proporc_13_fer > 0) {//para Saber qual coluna Crédito ou Débito
...
//12) Prêmio Os Melhores
                    if($id_funcionario_campeao_os_melhores == $id_funcionario_loop) {
                        $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Prêmio "Os Melhores"', 1, 0, 'L');
                        $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Campeão', 1, 0, 'C');
                        
                        //Essa variável sempre será o Prêmio de Vice-Campeão + 50% ...
                    ...
                    if($id_funcionario_vice_campeao_os_melhores == $id_funcionario_loop) {
                        $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Prêmio "Os Melhores"', 1, 0, 'L');
                        $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Vice-Campeão', 1, 0, 'C');
...
//13) SubTotal Prêmio de Vendas s/ Proporcionalidade
                    if($campos_funcionario_holerith[0]['subtotal_premios'] > 0) {
                        $pdf->Cell($GLOBALS['ph']*30, 4.5, 'SubTotal Prêmio de Vendas s/ Proporc.', 1, 0, 'L');
                        $pdf->Cell($GLOBALS['ph']*30, 4.5, '', 1, 0, 'C');
...
//14) Outros Rendimentos Proporcionais
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, 'Outros Rend. + Prêmio s/ Proporc.', 1, 0, 'L');
                    $pdf->Cell($GLOBALS['ph']*30, 4.5, $dias_horas_trabalhadas, 1, 0, 'C');
...
//Quando o Valor for = 0, então eu não printo essa linha ...
                    if($outros_rend_prop == 0) {
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                    }else if($outros_rend_prop > 0) {//para Saber qual coluna Crédito ou Débito
...
/***********************************************************************************************/
                if($opt_descontar == 'PF') {
                    ...
                    $pdf->Cell($GLOBALS['ph']*60, 4.5, 'Sub-Total à Receber', 1, 0, 'L', 1);
//Quando o Valor for = 0, então eu não printo essa linha ...
                    if($sub_total_receber_rs == 0) {
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R', 1);
                        $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R', 1);
                    }else if($sub_total_receber_rs > 0) {//para Saber qual coluna Crédito ou Débito
...
/***************************************************************************************************/
/***********************Daqui em Diante eu Exibo todos os Vale(s) do Funcionário na Data de Holerith 
especificada pelo usuário***************************************************************************/
            $pdf->SetFont('Arial', '', 9);
            
            //Só exibo os Vales ativos ...
            ...
            $total_descontos = 0;//Controle do Total de Vale(s) do Funcionário Independente do Tipo de Vale ...
            //Disparo do Segundo Loop ...
            for($j = 0; $j < $linhas_vales_dps; $j++) {
                //Se o Tipo do Vale = Credito Consignado, então eu concateno com a Financeira e N.º de Parcelas ...
                $parcelamento = ($campos_vales_dps[$j]['tipo_vale'] == 14) ? ' '.$campos_vales_dps[$j]['financeira'].' ('.$campos_vales_dps[$j]['parcelamento'].') ': '';
                //Se o Tipo do Vale = Avulso, então eu concateno com a observação ao lado
...
//Quando o Valor for = 0, então eu não printo essa linha ...
                if($vales == 0) {
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                }else if($vales > 0) {//para Saber qual coluna Crédito ou Débito
            ...
            /*Do total de Descontos eu também abato o Valor Pensão Alimentícia e Valor Credito 
...
/***********************************************************************************************/
            if($opt_descontar == 'PD') {
                $pdf->SetFont('Arial', 'B', 9);
//11) Observação
                $pdf->MultiCell($GLOBALS['ph']*90, 5, 'OBSERVAÇÃO: '.$observacao_func_holerith, 1, 'L');
            }
//Total de Vale do Funcionário ...
            $pdf->SetFont('Arial', 'B', 9);
...
//Quando o Valor for = 0, então eu não printo essa linha ...
            if($total_descontos == 0) {
                $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R', 1);
                $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R', 1);
            }else if($total_descontos > 0) {//para Saber qual coluna Crédito ou Débito
...
//Somente se for Desconto PF que eu exibo essa linha ...
/***********************************************************************************************/
            if($opt_descontar == 'PF') {//Cálculo do Total à Receber PF em R$
                $total_receber_pf = $sub_total_receber_rs + $total_descontos;
                $pdf->Cell($GLOBALS['ph']*60, 4.5, 'Total à Receber PF', 1, 0, 'L',1);
//Quando o Valor for = 0, então eu não printo essa linha ...
                if($total_receber_pf == 0) {
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                }else if($total_receber_pf > 0) {//para Saber qual coluna Crédito ou Débito
...
//Tenho esse lembrete p/ que fique bem claro p/ o usuário ...
                $valor_negativo = ($total_receber < 0) ? '(Valor Negativo)   ' : '';
//Cálculo do Total à Receber em R$ ...
                $pdf->SetFont('Arial', 'B', 9);
                $pdf->Cell($GLOBALS['ph']*60, 4.5, 'Total à Receber (Arredondado a cada R$ 0,25) ', 1, 0, 'L',1);
//Quando o Valor for = 0, então eu não printo essa linha ...
                if($total_receber == 0) {
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 0, 'R');
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R');
                }else if($total_receber > 0) {//para Saber qual coluna Crédito ou Débito
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, number_format($total_receber, 2, ',', '.'), 1, 0, 'R', 1);
                    $pdf->Cell($GLOBALS['ph']*15, 4.5, '', 1, 1, 'R', 1);
                }else {
                    /*Somente no vale "PF" que o ERP fará esse controle p/ enviar e-mail p/ a Dona Sandra "Diretora" 
...
//Aqui eu guardo o Valor Total à Receber na Tabela de Funcionários vs Holeriths (Créditos) ...
...
//Primeiro eu verifico se já existe esse id_funcionario na Tabela ...
                ...
                if(count($campos_holerith) == 0) {//Ainda não existe, então eu gravo na Base de Dados ...
...
//Exibição dos Dados Bancários ...
                $pdf->SetFont('Arial', '', 9);
                if(empty($cadastro_banco[$campos[$i]['cod_banco']][0])) {//Certifico que ele não foi deletado, Luis ..
                    $banco = $cadastro_banco[$campos[$i]['cod_banco']][1];
                }else {
                    $banco = $cadastro_banco[$campos[$i]['cod_banco']][0];
                }
                $pdf->Cell($GLOBALS['ph']*90, 4.5, 'Banco - '.$banco.' / Agência - '.$campos[$i]['agencia'].' / Conta Corrente - '.$campos[$i]['conta_corrente'], 1, 1, 'L',1);
            }
/******************************************Desconto PF******************************************/
//Somente se for Desconto PF que eu exibo essa linha ...
/***********************************************************************************************/
...
/******************************************************************************/
if(!empty($funcionarios_com_vales_negativos) && $_SESSION['id_funcionario'] != 98) {//Somente o Funcionário "98 Dárcio" que nunca gerará esse Tipo de E-mail ...
    $mensagem_email = 'Saldo negativo Holerith com vencimento em '.data::datetodata($_GET['cmb_data_holerith'], '/').'.<br/><br/>';
    $mensagem_email.= 'Segue abaixo a relação do(s) Funcionário(s) que possuem Vale Negativo: <br/>';
    $mensagem_email.= $funcionarios_com_vales_negativos.'<br/><br/>';
    $mensagem_email.= 'Favor gerar os vales para os funcionários na função => ';
    $mensagem_email.= 'Menu Depto Pessoal / Folha de Pagto / Holerith / Gerenciar Vales / Botão Incluir Vale / Opção ';
