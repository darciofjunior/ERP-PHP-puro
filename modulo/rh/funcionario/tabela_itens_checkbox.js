...
//Aqui é um controle para poder selecionar o checkbox principal
    ...
    for(i = 0; i < elementos.length; i++) {
        if(elementos[i].name == 'chkt_vale_transporte[]') {
            //Somente p/ Vales Transportes q estão checados
            if(elementos[i].checked == true) total_itens_vts_marcados++
            total_itens_vts++
        }
    }
//Significa que todos os itens de Vales Transportes estão marcados
...
function selecionar(form, campo, totallinhas, cor_origem) {
    ...
    for (var i = 0; i < linhas; i++) {
        if(campo_sel.checked) {//Aqui são os checkbox, se estiver checado
