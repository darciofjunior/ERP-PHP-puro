...
for($i = 0; $i < $linhas; $i++) {
    ...
    $sql = "SELECT nfs.`id_cliente_contato` 
    ...
    $id_cliente_contato     = (count($campos_conta) == 1) ? $campos_conta[0]['id_cliente_contato'] : 'NULL';
    ...
    //$sql = "INSERT INTO `follow_ups` (`id_follow_up`, `id_cliente`, `id_cliente_contato`, `id_representante`, `id_funcionario`, `identificacao`, `origem`, `observacao`, `data_sys`) VALUES (NULL, '".$campos[$i]['id_cliente']."', $id_cliente_contato, $id_representante, $id_funcionario, '".$campos[$i]['id_conta_receber']."', '4', '".addslashes($campos[$i]['observacao'])."', '".date('Y-m-d H:i:s')."') ";
...
//Aqui eu já passo o índice do próximo ...
    window.location = 'script_follow_ups12.php?indice=<?=++$indice;?>'
</Script>