...
return migrar_fci_albafer_para_pas('migrar_fci_albafer_para_pas.txt');

function migrar_fci_albafer_para_pas($arquivo, $separacao) {
    if(file_exists($arquivo) && is_readable($arquivo)) {
        ...
        for($i = 0; $i < count($linhas); $i++) {
            ...
            $sql = "UPDATE `produtos_acabados` SET `fci_albafer` = '$fci_albafer' WHERE `referencia` = '$referencia' LIMIT 1 ";
            echo $sql.';<br/><br/>';
        }
        flush();
