...
return migrar_novos_produtos_precos('migrar_novos_produtos_precos.txt');
    ...
    if (file_exists($arquivo) && is_readable($arquivo)) {
        ...
        for ($x = 0; $x < count($linhas); $x ++) {
            ...
            if(count($campos_pipa) == 1) {
                /*Agora com o PI na mão eu consigo entrar na Lista de Preço do Fornecedor Winner "2162" 
                e atualizar o seu preço ...*/
                $sql = "UPDATE `fornecedores_x_prod_insumos` fpi SET `preco_faturado` = '$preco_lista', 
                        `data_sys` = '".date('Y-m-d')."' 
