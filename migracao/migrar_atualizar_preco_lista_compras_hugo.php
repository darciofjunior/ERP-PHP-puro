...
return migrar_novos_produtos_precos('migrar_atualizar_preco_lista_compras_hugo.txt');

/*
Trunca os preços da lista do Fornecedor ...

UPDATE `fornecedores_x_prod_insumos` fpi 
INNER JOIN `produtos_acabados` pa ON pa.`id_produto_insumo` = fpi.`id_produto_insumo` 
AND pa.`referencia` <> 'ESP'
SET fpi.`preco_faturado` = '0', fpi.`preco` = '0' 
WHERE fpi.`id_fornecedor` = '425'*/
