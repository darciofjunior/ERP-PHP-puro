...
return migrar_novos_produtos('migrar_estoque_tdc_fornecedor.txt');
        ...
        for ($i = 0; $i < count($linhas); $i ++) {
            ...
            if(count($campos_produto_acabado) == 1) {
                ...
                if(count($campos) == 0) {//PA não existente na Tabela de Estoque ...
                    $sql = "INSERT INTO `estoques_acabados` (`id_produto_acabado`, `qtde_fornecedor`) VALUES ('$id_produto_acabado', '$qtde_fornecedor') ";
                }else {//PA já existente na Tabela de Estoque ...
                    //Primeiro Zero o Registro de Estoque p/ gerar futuros erros de apresentação ...
                    $sql = "UPDATE `estoques_acabados` SET `qtde_fornecedor` = '0' WHERE `id_produto_acabado` = '$id_produto_acabado' LIMIT 1 ";
                    bancos::sql($sql);
                    
                    $sql = "UPDATE `estoques_acabados` SET `qtde_fornecedor` = '$qtde_fornecedor' WHERE `id_produto_acabado` = '$id_produto_acabado' LIMIT 1 ";
                }
                echo '<br/>'.$sql;
                bancos::sql($sql);
            }
