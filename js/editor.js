...
/* Observação checkbox para visualizar o html*/
+	"<TR><TD><INPUT class=checkbox TYPE=checkbox ID=cbMode ONCLICK=\"setMode(!this.checked)\"> &nbsp;&nbsp;</TD>"
...
this.szHTML			= this.PrepareHTML()
//this.autoSize		= false // aumenta o tamanho da janela
break
case "BackColor": 
...
{
sz  +=  "<IMG TABINDEX=" + (i+1) + " ONCLICK=\"parent._CEmoticonRenderer_Select(this)\" CLASS=emoticon SRC=\"" + L_EMOTICONPATH_TEXT + arEmoticons[i] + "\" WIDTH=19 HEIGHT=19 HSPACE=3 VSPACE=3>"
if ((i+1)%8==0) sz+="<BR>"
}			
if (i%8!=0) sz+="<BR>"
for (var i=0; i < arBigEmoticons.length; i++) 
{
sz  +=  "<IMG TABINDEX=" + (i+arEmoticons.length) + " ONCLICK=\"parent._CEmoticonRenderer_Select(this)\" CLASS=emoticon SRC=\"" + L_EMOTICONPATH_TEXT + arBigEmoticons[i] + "\" WIDTH=29 HEIGHT=29 HSPACE=2 VSPACE=2>"
if ((i+1)%6==0) sz+="<BR>"
}			
sz+="</DIV>"
return sz
