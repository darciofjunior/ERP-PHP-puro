    ...
    foreach ( $tables as $group => $table ) {
        if ( isset( $table['is' . $sep . 'group'] ) ) {
            ...
            echo ' id="icon_' . htmlspecialchars( $table_db . '.' . $table['Name'] ) . '"'
                ...
                .'<a style="font-family: Verdana, Geneva, Arial, Helvetica, sans-serif; font-size: 10px;"  href="' . $href . '" title="' . $table['Comment']
                .' (' . PMA_formatNumber( $table['Rows'], 0 ) . ' ' . $GLOBALS['strRows'] . ')"'
                .' id="' . htmlspecialchars( $table_db . '.' . $table['Name'] ) . '"><font size="2">'
                . htmlspecialchars( $table['disp_name'] ) . '</font></a>';
            echo '</li>' . "\n";
        }
    }
    echo '</ul>';
